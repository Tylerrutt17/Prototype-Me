...
export function findAll(userId: string, filters?: { status?: string }) {
  ...
  if (filters?.status) conditions.push(eq(directive.status, filters.status));
...
export function insertHistory(directiveId: string, action: string, payload = "{}") {
  return db.insert(directiveHistory).values({ directiveId, action, payload }).returning().then((r) => r[0]!);
}
